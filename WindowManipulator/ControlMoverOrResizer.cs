...
internal class ControlMoverOrResizer
{
    ...
    private const int gapAllowed = 9;
    private static void UpdateMouseEdgeProperties(Control control, Point mouseLocationInControl)
    {
        ...
        MouseIsInLeftEdge = Math.Abs(mouseLocationInControl.X) <= gapAllowed;
        MouseIsInRightEdge = Math.Abs(mouseLocationInControl.X - control.Width) <= gapAllowed;
        MouseIsInTopEdge = Math.Abs(mouseLocationInControl.Y) <= gapAllowed;
        MouseIsInBottomEdge = Math.Abs(mouseLocationInControl.Y - control.Height) <= gapAllowed;
    }
